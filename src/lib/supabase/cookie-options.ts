 ...
 *
 * ## 배경
 * middleware(`updateSession`)와 `completeOnboarding` Server Action 양쪽이
 * 동일한 캐싱 쿠키를 set한다. 기존에는 두 곳에 옵션이 하드코딩되어 있어
 * 한 곳만 수정되면 캐싱이 깨지는 동기화 위험이 있었음. 단일 출처로 분리.
 *
 * ## 성격: UX 캐싱 쿠키 (보안 경계 아님)
 * middleware가 매 요청마다 `user_profiles.onboarding_completed`를 조회하지
 * 않도록 1시간 캐싱. 실제 접근 제어는 `(dashboard)/layout.tsx` + middleware가
