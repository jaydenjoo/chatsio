...
- **원인**:
  1. `.env.local` 첫 줄 `# Created by Vercel CLI` 주석 + `VERCEL_OIDC_TOKEN` 키 존재 → "Vercel과 연결되어 있다"는 인상
  2. 그 인상을 검증하지 않고 "이미 등록되어 있을 것"이라는 가정으로 비약. 실제로는 과거 어느 시점에 `vercel link` + `vercel env pull`을 시도했다 잔재만 남고, 현재 Vercel에는 chatsio 프로젝트 자체가 없는 상태
  3. **추측을 단정형으로 표현**한 것이 핵심 실수. "Vercel에 등록되어 있다면 이미 X일 것" 같은 조건문이었으면 사용자가 자연스럽게 "아니, 등록 안 했어"로 정정 가능. "이미 X일 것"은 사용자가 본인의 멘탈 모델을 의심하게 만드는 표현
  4. 비개발자 사용자는 "AI가 단정한 사실"을 기본값으로 받아들이는 경향 — 정정하려면 추가 인지 비용 발생
- **해결**: 즉시 사과 + 정정 ("`.env.local`에 키가 있다고 해서 Vercel에 등록됐다는 보장이 전혀 없습니다. 두 개는 별개입니다. 정정합니다"). 가능한 시나리오 3개 + 확인 명령(`ls .vercel`, `vercel whoami`) 제시. Jayden이 작업 범위를 옵션 3(잔재 키 정리만)으로 좁힘 → 안전한 범위에서 마무리
- **규칙**:
  1. **외부 시스템 상태(Vercel/Supabase/n8n/GitHub)는 로컬 파일의 흔적이 아니라 해당 시스템의 직접 조회로만 확인** — `.env.local`에 키 존재 / `package.json`에 의존성 존재 / `.vercel/` 폴더 존재 등은 모두 "한 번이라도 시도된 흔적"일 뿐 "현재 등록/연결 상태"의 증거가 아님. 등식 금지: `로컬 파일 흔적 = 외부 등록`
  2. **추측을 단정형으로 표현 금지** — "이미 등록되어 있을 것", "이미 설정되어 있을 것", "이미 활성화되어 있을 것" 같은 표현은 사용자의 멘탈 모델을 흔든다. 대신 조건문 사용: "만약 Vercel에 등록되어 있다면 ~", "Vercel 등록 상태는 `vercel env ls`로 먼저 확인 후 ~"
  3. **단언하기 전에 확인 명령 제안** — 답변에 "이미 ~되어 있을 것"이라고 쓰고 싶어지면, **그 추측을 검증하는 명령(`vercel env ls`, `vercel whoami` 등)을 답변 안에 포함**시켜서 사용자가 답변을 따르기 전에 검증하도록 유도. 추측만 던지지 말 것
  4. **비개발자 사용자에게는 "정정 비용 0"을 명시적으로 신호** — "위 표는 제 추측입니다. 실제 등록 상태는 `vercel env ls`로 확인 후 알려주세요"를 첨가. 사용자가 "내가 잘못 알고 있나?"가 아니라 "AI 추측이니까 검증해야겠다"로 인지하게 함
  5. **이 규칙은 Session #23 [AI-Pitfall] "외부 서비스 UI 가이드..." 교훈의 짝** — 전자는 SaaS UI(분기별 변경)에 대한 것, 이번은 외부 시스템 등록 상태(사용자 액션 이력)에 대한 것. 둘 다 핵심은 "내 기억/추측 → 단정 금지, 사용자 화면/시스템 → 진실"
- **컨텍스트**: Session #24 `.env.local` 정리 작업 중 발생. 다행히 코드 실수가 아닌 대화 단계에서 정정됐고, 만약 단정형 안내를 그대로 따랐다면 Jayden이 "이미 있다고 하니까 나머지만 추가하면 되겠지" 하고 잘못된 절차를 따랐을 위험. 결과적으로 작업 범위를 옵션 3(안전한 범위)으로 좁혀서 정리. 이번에 정정됐으니 다음에는 단정형 추측 자체를 만들지 말 것
